...
class PredictorService:
    """
    Inference service that loads binary classification LSTM models and scalers into memory
    to forecast stock price movement direction (UP / DOWN, BUY / SELL) and confidence scores.
    """
    def __init__(self):
        ...
        # Pre-load fallback default classifier files if available upon initialization
        self._load_default_files()

    def _load_default_files(self):
        """
        Loads general fallback stock_classifier.h5 and scaler.pkl if they exist.
        """
    ...
    def has_model_files(self, ticker: str) -> bool:
        """
        Checks if binary classification model files exist for the given ticker (or general fallback).
        """
        ...
        # Check ticker-specific files or general default files
    ...
    def load_ticker_model(self, ticker: str) -> bool:
        """
        Loads the binary classification model, scaler, and validation metrics for the ticker into RAM.
        """
        ticker = ticker.upper().strip()
        
        # If already cached in memory, return True
    ...
    def predict_direction(self, ticker: str, recent_prices: list) -> dict:
        """
        Takes a list of 60 recent closing prices, normalizes input, runs sigmoid classification inference,
        and returns a directional prediction dictionary: {"confidence_up": float, "prediction": string, ...}.
        """
        # Handle positional parameter variation (ticker, recent_prices) or (recent_prices)
